...
export default function Home() {
  ...
  const [pendingSettings, setPendingSettings] =
    useState<ProcessSettings | null>(null);
  const [loadingPreview, setLoadingPreview] = useState(false);

  const handleUploaded = useCallback((id: string, dur: number, url: string) => {
    setJobId(id);
    setDuration(dur);
    setPreviewUrl(url);
    setStep("settings");
  ...
  const handleStartProcessing = useCallback(
    async (settings: ProcessSettings) => {
      ...
      try {
        ...
        setError(
          e instanceof Error ? e.message : "字幕プレビューに失敗しました",
        );
      } finally {
  ...
  const progressPct =
    currentIdx === 0 ? 0 : (currentIdx / (stepOrder.length - 1)) * 100;
    ...
    <main className="min-h-screen flex flex-col bg-gradient-to-b from-gray-950 via-gray-900 to-gray-950">
      ...
      <header className="py-8 text-center">
        ...
        <p className="text-gray-400 mt-2 text-sm">
          TikTok / Instagram Reels 用動画を AI で一気に作成
        </p>
      </header>
