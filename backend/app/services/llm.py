...
**短い subword 列の書き換え:**
- 5 文字以下で意味が通らない segment (例:「客 事への 食」「ボメ 康」「悪い要」) は
  周辺の前後 segment の文脈から推測して、 意味の通る日本語に書き換えて良い
- 「客 事への 食」 → 「お客様が」 のように大幅な短縮も許容
- ただし統合・分割は禁止 (時刻整合性を守る)

**長さ制約:**
- 各セグメントは元のセグメントの -70% 〜 +50% 程度の長さ
- 短い subword 列の修正は -70% まで縮めて良い
- 大きく増やさない(+50% を超えない、 読み切れなくなる)
...
def correct_transcript_segments(segments: list[str], video_context: str = "") -> list[str]:
    ...
    for item in parsed.corrections:
        ...
        # 文字数が元の 2.5倍超 / 15文字以上長い場合は LLM がセグメントを統合した可能性が高いので破棄
        # (短い subword 列 5文字 → 12文字、 10文字 → 25文字 までの書き換えを許容)
        if len(new_text) > max(int(len(original) * 2.5), len(original) + 15):
            rejected += 1
...
2. **likely_mishearings**: この動画固有の誤認識候補リスト。
   ...
   - **subword 化された短い断片**: 「ダイエットの食事」が「事への 食」と切れていたら
     {"wrong":"事への","right":"の"}
   - **意味不明な並び**: 「ボディーメイク 健康」が「ボメ 康」と subword 欠落していたら
     {"wrong":"ボメ","right":"ボディーメイク"}
   - **反義語的誤り**: 「重要」が「悪い要」のように音響近似で誤った場合
     {"wrong":"悪い要","right":"重要"}
   - **1 字目欠落**: 「自律神経」が「こ律神経」 (「じ」→「こ」誤認識)
     {"wrong":"こ律","right":"自律"}
   ルール:
   - 文脈から **明らかに誤り** と判断できるもののみ
   - 0〜15 個程度。自信が無いものは含めない
   - 短い置換単位(1〜6文字)を優先(長い文の置換は副作用が大きい)
   - 助詞だけ・句読点だけの置換は禁止
   - **subword 化された短い断片や意味不明な並び** を特に積極的に検出する
