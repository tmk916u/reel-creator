...
- [x] 1.1 `video.py` の Stage 5b で `cut2_generated=True` 経路に hybrid 補完を追加:
...
- [x] 1.2 同じく `cut2_generated=False` (施策F 未発動)経路にも補完を追加:
  - 字幕用 = words_cut (cut.mp4 ベース) の前に 1段目 補完を hybrid prepend
  - 1段目 → cut.mp4 内時刻の 1 段 remap だけで済む

## 2. テスト追加

- [x] 2.1 既存 112 件のテストが PASS することを確認
- [x] 2.2 `backend/tests/test_video_router_stage5b.py` にテスト追加 (3 件):
...
- [x] 3.1 `test-videos/seitai_food.mov` を `/api/process` で再処理(tight preset) → job f4703bef
- [x] 3.2 `python backend/scripts/measure_quality.py f4703bef-cef3-4332-ab64-c74f97ebdf3b` を実行
- [x] 3.3 字幕冒頭 Dialogue を目視確認: 「客 食事の話をしようと思います」「結論から言うと一番大事なのはメンタルです」が出現 ✅
- [x] 3.4 動画長 103.2s、 処理時間 260s、 退行なし

## 4. baseline.md / HANDOVER.md 更新

- [x] 4.1 `establish-quality-line/baseline.md` に新 job f4703bef 列を追加
- [x] 4.2 概要(機械測定 4/6、 全体合格率 9/14 推定)を更新
- [x] 4.3 `fix-third-stage-asr-leading-miss` を「✅ 解決済」 とマーク (本 change 完了で)
- [ ] 4.4 `.planning/HANDOVER.md` に hybrid 補完を反映

## 5. archive 準備

- [x] 5.1 全タスクの完了確認
- [x] 5.2 字幕冒頭に動画冒頭発話が出ていることを確認
