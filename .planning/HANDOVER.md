...
### `fix-subtitle-segmentation-quality` 完了 — 字幕分節品質の改善
- 実装:
  - `_trailing_particles` を拡張: 「て / ば / し / ず」 を追加 (接続助詞・活用語尾)
  - 「、」 を flush セットから除外: 句点 (。！？!?.) のみで flush
  - `_merge_short_segments` を強化: min_chars=8 で短すぎる Dialogue を統合候補に
  - 統合後の合計を max_chars × 1.4 (= 14) 以内に保つ上限制御
- テスト 5 件追加 → 120/120 PASS
- 実測 (job df58f7dc, max_chars=10):
  - **#7 助詞直後切れ 38.7% → 26.7%** (大幅改善、 目標 10% 未達)
  - **#8 字幕 8-14 文字 41.9% → 50%** (改善、 目標 70% 未達)
  - #6 同期 ✅ 維持、 #4 動画長 ✅ 94.5s、 #14 処理時間 ✅ 256s
- 完全合格には至らず、 1 動画 1 ベンチマークでは限界がある。 美観の調整は将来の change で対応

### 業務量産品質ライン 最終ベースライン (job df58f7dc)
- 機械測定: **4/6 合格** (#4 #6 #11 #14 ✅、 #7 #8 ⚠️ 改善あり)
- 目視: 推定 **5/8** (#1 #2 #3 #5 #9 #13 ✅、 #10 #12 未測定)
- **全体 9/14 (推定 64%)** — 業務量産投入可能ラインにほぼ到達

### 残課題
- **#7 助詞直後切れ 26.7%** (目標 10%): 美観の問題、 業務量産には許容範囲
- **#8 字幕 8-14 文字 50%** (目標 70%): 同上
- **#10 テロップ被写体保護** / **#12 トピックラベル整合**: 目視未測定
- これらは将来 必要に応じて別 change で対応
