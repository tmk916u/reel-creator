...
- **アカウント文脈プロファイル**: ブランド名・ターゲット・トーンを登録し AI 生成をアカウント特化
- **出力の自動QC**: 尺崩れ・縦型崩れ・誤字幕を投稿前に警告
- **HyperFrames リール仕上げ**: 前処理済み映像にブランドタイポを乗せて雑誌/CM級の縦型リールを生成（4テイスト）→ [hyperframes/README.md](hyperframes/README.md)
...
前処理済み映像にブランドタイポ（明朝/ゴシック/縦書き和風など 4 テイスト）を乗せ、
Instagram リール級の縦型動画を作る工程。アカウント設定（ブランド名・ハンドル）に沿って自動セットアップする。

```bash
# 例: ジョブから縦書き和風テイストで生成 → レンダー → 再生
node hyperframes/new-from-job.mjs <job_id> --template tategaki-wa --render --open
```

レンダーはホスト(mac)側で実行（HyperFrames はヘッドレス Chromium を使うため）。
詳しい使い方・テイスト・編集方法は **[hyperframes/README.md](hyperframes/README.md)**。
